...
describe('HeroesComponent', () => {
  ...
  const heroService = jasmine.createSpyObj('HeroService', ['getHeroes', 'creatHero', 'editHero', 'removeHero']);
  ...
  it('should display heroes', () => {
    ...
    expect(fixture.nativeElement.querySelectorAll('mat-card').length).toBe(heroTest.length);
    expect(fixture.nativeElement.querySelectorAll('li').length).toBe(2 * heroTest.length);
  });
  ...
  it('should remove hero when click remove button', async () => {
    ...
    const btn: HTMLButtonElement = fixture.nativeElement.querySelector('#btnRemove');
    btn.click();
    expect(removeHeroSpy.calls.any()).toBe(true, 'remove hero called');
    fixture.detectChanges();
    expect(fixture.nativeElement.querySelectorAll('mat-card').length).toBe(matLen - 1);
  });
